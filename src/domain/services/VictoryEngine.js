...
class VictoryEngine {
  static evaluateVictory(room) {
    ...
    const livingKingdom = players.filter(p => p.isAlive && p.faction === Faction.KINGDOM);
    const livingShadows = players.filter(p => p.isAlive && p.faction === Faction.SHADOW);
